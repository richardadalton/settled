...
impl SettledClient {
    ...
    pub async fn append(&mut self, key: Vec<u8>, data: Vec<u8>) -> Result<AppendResult, ClientError> {
        let r = self.inner.append(Request::new(proto::AppendRequest { key, data })).await?.into_inner();
    ...
    pub async fn get(&mut self, seq: u64) -> Result<Entry, ClientError> {
        let r = self.inner.get(Request::new(proto::GetRequest { seq })).await?.into_inner();
    ...
    pub async fn get_latest(&mut self, n: u32) -> Result<Vec<Entry>, ClientError> {
        let r = self.inner.get_latest(Request::new(proto::GetLatestRequest { n })).await?.into_inner();
    ...
    pub async fn get_sth(&mut self, tree_size: u64) -> Result<SignedTreeHead, ClientError> {
        let r = self.inner.get_sth(Request::new(proto::GetSthRequest { tree_size })).await?.into_inner();
    ...
    pub async fn inclusion_proof(&mut self, seq: u64, tree_size: u64) -> Result<InclusionProofResult, ClientError> {
        let r = self.inner.inclusion_proof(Request::new(proto::InclusionProofRequest { seq, tree_size })).await?.into_inner();
    ...
    pub async fn consistency_proof(&mut self, old_size: u64, new_size: u64) -> Result<ConsistencyProofResult, ClientError> {
        let r = self.inner.consistency_proof(Request::new(proto::ConsistencyProofRequest { old_size, new_size })).await?.into_inner();
