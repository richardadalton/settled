...
impl SettledClient {
    ...
    pub async fn append(
        ...
        let res = self
            .inner
            .append(Request::new(AppendRequest { key, data }))
            .await?;
        Ok(res.into_inner())
    }
    ...
    pub async fn get_latest(&mut self, n: u32) -> Result<GetLatestResponse, ClientError> {
        let res = self
            .inner
            .get_latest(Request::new(GetLatestRequest { n }))
            .await?;
        Ok(res.into_inner())
    }
    ...
    pub async fn get_sth(&mut self, tree_size: u64) -> Result<GetSthResponse, ClientError> {
        let res = self
            .inner
            .get_sth(Request::new(GetSthRequest { tree_size }))
            .await?;
        Ok(res.into_inner())
    }
    ...
    pub async fn inclusion_proof(
        ...
        let res = self
            .inner
            .inclusion_proof(Request::new(InclusionProofRequest { seq, tree_size }))
            .await?;
        Ok(res.into_inner())
    }
    ...
    pub async fn consistency_proof(
        ...
        let res = self
            .inner
            .consistency_proof(Request::new(ConsistencyProofRequest { old_size, new_size }))
            .await?;
        Ok(res.into_inner())
    }
}
